...
//!
//! Avoids macOS 14+ SCScreenshotManager APIs and uses the system
//! `screencapture` utility instead. This preserves window/display screenshot
//! functionality on macOS 12.7 while keeping the public API expected by the
//! rest of cua-driver.
...
pub fn screenshot_window_bytes(window_id: u32) -> anyhow::Result<Vec<u8>> {
    ...
    let path = capture.file.to_string_lossy().into_owned();
    let output = Command::new("screencapture")
        .args(["-l", &window_id.to_string(), "-x", "-o", &path])
        .output()?;
