...
# The 0.28.x permission gate relies on fresh helper-process probes that are
# unreliable on Monterey. Real TCC enforcement remains active in Accessibility
# and CoreGraphics; this only skips the newer startup gate state machine.
...
DRIVER_VERSION="$($PERSISTENT_BIN --version | awk '{print $NF}')"
[[ -n "$DRIVER_VERSION" ]] || DRIVER_VERSION="0.28.1"
...
<dict>
  ...
  <key>CFBundleShortVersionString</key><string>$DRIVER_VERSION</string>
  <key>CFBundleVersion</key><string>$BUNDLE_VERSION</string>
  <key>LSBackgroundOnly</key><true/>
  <key>LSEnvironment</key>
  <dict>
    <key>CUA_DRIVER_RS_PERMISSIONS_GATE</key><string>0</string>
    <key>CUA_DRIVER_RS_TELEMETRY_ENABLED</key><string>0</string>
  </dict>
</dict>
</plist>
PLIST
...
# Launch by explicit bundle path because Monterey installs the compatibility app
# under ~/Applications rather than /Applications.
open -n -g "$APP_ROOT" --args --no-permissions-gate serve >/dev/null 2>&1 || true
sleep 2
