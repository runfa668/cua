...
# Hermes prefers the Browser Use CLI when it is runnable. Install it in an
# isolated uv-managed Python 3.12 environment when missing so Monterey's system
# Python remains untouched.
if ! command -v browser-use >/dev/null 2>&1; then
    echo "==> Browser Use CLI not found; installing a local Hermes-compatible copy"
    if ! command -v uv >/dev/null 2>&1; then
        command -v curl >/dev/null 2>&1 || { echo "warning: curl is unavailable; skipping Browser Use install" >&2; }
    ...
    if command -v uv >/dev/null 2>&1; then
        uv tool install --python 3.12 --upgrade --force browser-use || \
            echo "warning: automatic Browser Use installation failed; cua-driver setup will continue" >&2
    fi
fi

# uv tool binaries normally land in ~/.local/bin. Refresh PATH before checks.
export PATH="$INSTALL_DIR:$HOME/.cargo/bin:$PATH"
...
if command -v browser-use >/dev/null 2>&1; then
    ...
    echo "warning: Browser Use CLI is still unavailable; Hermes will fall back to its built-in browser tools."
fi
